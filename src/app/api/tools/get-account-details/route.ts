...
import { account_balance, getAccountForAddress } from "@/app/near-intent/actions/zec";
...
async function get_zec_balance(accountId: string) {
    ...
    const account = await getAccountForAddress(settings.zcashaddr);
    if (!account)
    {
        return 0;
    }
    const [tp, sh] = await account_balance((account));
    return tp + sh;
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return Response.json({token_balance_wallet: token_balances, satoshi: btc_balance, zec: zec_balance, token_balance_defuse: defuse_balances},  { status: 200 });
  } 
  catch (error) {
    console.error("Error fetching balances:", error);
    return Response.json({ error: "Internal server error" }, { status: 500 });
